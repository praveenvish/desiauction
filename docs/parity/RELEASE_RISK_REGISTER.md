...
| R-D1 | Migration / rollback | This branch added **migrations `0015`–`0026`**, among them **`0019_tournaments`** (2026-07-23) — the schema-free rollback posture no longer holds and a rollback now needs a down-path or a restore. `0019` renames `seasons`→`tournaments`, drops `year`, adds a unique `slug`, and renames `competitions.season_id`→`tournament_id`; it renames rather than drops, so the RLS policy and tenant index carry over (verified post-apply: `tournaments_tenant` retains both USING and WITH CHECK). Migrations `0000–0019` apply green on PG17 from empty. | High if wrong | Low | High | No down-path exists or is planned — migrations are forward-only by policy (`docs/auction/RUNBOOKS.md` §10). The answer is expand/contract for new migrations plus a recorded pre-deploy restore point, and a rollback drill before release. Both are unclosed: the restore point needs the founder-held managed Postgres. | `RUNTIME` (apply from empty) / `NONE` (rollback drill) | **Yes — rollback is no longer free** |
...
| Rollback validated | ❌ **no** — the "schema-free by construction" line that stood here contradicted R-D1 in the same document. Migrations are forward-only with no down path; `0019_tournaments` renames a table and drops a column, and 0015–0026 shipped after RC-1. No drill has been run and the restore point R-D1 falls back on is unprovisioned ([DEPLOYMENT §Rollback](../operations/DEPLOYMENT.md#rollback)) |
