...
|---|---|---|---|---|---|---|---|---|
| R-E1 | **E2E — critical journeys** | The register / auction / settlement journeys are exercised by Playwright specs, but I could **not** obtain a green local run this session — the local e2e DB (`.env.local` → **:5433**) carries residue and the `public-registration` spec fails at pre-existing gates (onboarding, public-page state). | High (unverified core flows) | Medium | **High** | Run the suite in CI against an **ephemeral** Postgres (charter's documented CI posture); reseed/reset :5433 for local. | `WRITTEN` (harness boots + drives; suite not green locally) | **YES** |
