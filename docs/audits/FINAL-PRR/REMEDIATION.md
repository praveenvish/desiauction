...
1. ~~**Nothing ever derives an organization's first certification.**~~ **FIXED.** `runDailyOps` now certifies each org daily — the same cadence the attestation runs on, bounded to one row rather than one per reader, and above the period gate because certification is about whether the projections fold out of the events, not about period discipline. `followAllOrgs` derives an org's FIRST certification as soon as the runner sees it, so a new org is not unknown for a day: an existence check per org per tick, one certification per org ever. The check pins `scopeType` so it rides `audit_scope_idx` rather than sequentially scanning a table that only grows — measured, not assumed. The audit action is now one exported constant shared by the writer and its three readers, because that string is a contract between them and the last drift rendered a PASS as "not matched". Proven: 28 certifications existed (all seed); after one 25s runner pass, **1,084 orgs certified, every verdict PASS**.
