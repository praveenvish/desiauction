(() => {
  ...
  var HOLD_THRESHOLD = 5e5;
  var ring = makeRing($("#oring"), 66);
  ...
  function render(s, e) {
    ...
    $("#dot").className = "o-dot" + (s.conn === "live" ? "" : " bad");
    ...
    $("#stateLine").innerHTML = s.phase !== "live" ? '<span class="lead-them">Bidding closed</span>' : s.bids.length === 0 ? '<span class="lead-them">Awaiting opening bid</span>' : youLead ? '<span class="lead-you">You lead</span>' : `<span class="lead-them"><b>${teamName(s.teams, s.leader)}</b> leads</span>`;
    ring.update(s.endsAt, s.phase === "live" && s.conn === "live");
    $("#recent").innerHTML = s.bids.slice(-3).reverse().map((b) => `<div class="r"><span>${teamName(s.teams, b.teamId)}${b.teamId === you.id ? " (you)" : ""}</span><span class="a">${short(b.amount)}</span></div>`).join("");
    const btn2 = $("#bid");
    const price = s.phase === "live" && s.player ? nextBid(s) : 0;
    const needHold = price >= HOLD_THRESHOLD;
    const gate = s.phase !== "live" ? "Bidding is closed" : s.conn !== "live" ? "Reconnecting \u2014 bids paused" : youLead ? "You hold the top bid" : price > left ? `Beyond your purse (${short(left)} left)` : "";
    btn2.disabled = !!gate;
    btn2.innerHTML = gate && s.phase === "live" && youLead ? `You lead at ${short(s.current)}` : s.phase === "live" ? `Bid ${short(price)}${needHold ? '<span class="sub">HOLD TO CONFIRM</span>' : ""}` : "Waiting for next lot";
    ...
    if (e.type === "bid") {
      ...
      if (wasLeading && s.leader !== you.id) {
        live.textContent += ". You were outbid.";
        if (navigator.vibrate && !reduced()) navigator.vibrate(30);
      }
    }
  ...
  function handleConn(s) {
    ...
    if (s.conn === "offline") {
      ...
      $("#resyncFacts").innerHTML = `Purse <span class="money">${short(you.purse - you.spent)}</span> \xB7 ` + (S?.phase === "live" ? `${S.leader === you.id ? "you lead" : teamName(S.teams, S.leader) + " leads"} at <span class="money">${short(S.current)}</span>` : "no lot live");
      steps[2].classList.add("done");
      setTimeout(() => panel.classList.remove("on"), reduced() ? 900 : 1600);
      live.textContent = "Reconnected. " + $("#resyncFacts").textContent;
    }
  }
  ...
  var startPress = (e) => {
    ...
    const tick = () => {
      ...
      if (k >= 1) {
        endPress();
        commit();
      } else holdRaf = requestAnimationFrame(tick);
    };
