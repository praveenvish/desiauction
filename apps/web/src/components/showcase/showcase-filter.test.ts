...
describe("filterSortPlayers", () => {
  it("filters by status", () => {
    expect(
      filterSortPlayers(P, { query: "", filter: "sold", sort: "number" }).map((p) => p.number),
    ).toEqual(["2"]);
    expect(
  ...
  it("free-text matches name (case-insensitive) or number", () => {
    expect(
      filterSortPlayers(P, { query: "amit", filter: "all", sort: "number" }).map((p) => p.name),
    ).toEqual(["amit shah", "Amit Rao"]);
    expect(
      filterSortPlayers(P, { query: "10", filter: "all", sort: "number" }).map((p) => p.number),
    ).toEqual(["10"]);
  });

  it("sorts numerically ('2' before '10'), by name, and by status", () => {
    expect(
      filterSortPlayers(P, { query: "", filter: "all", sort: "number" }).map((p) => p.number),
    ).toEqual(["1", "2", "10"]);
    expect(
      filterSortPlayers(P, { query: "", filter: "all", sort: "name" }).map((p) => p.name),
    ).toEqual(["Amit Rao", "amit shah", "Zara Khan"]);
    expect(
      filterSortPlayers(P, { query: "", filter: "all", sort: "status" }).map((p) => p.status),
    ).toEqual(["available", "available", "sold"]);
  });
