...
export interface DeskAsk {
  ...
  /** Decided here, on the server's clock, so the desk never reads a clock while rendering. */
...
export async function reviewDesk(db: Db = systemDb): Promise<ReviewDesk> {
  ...
  return {
    ...
    asks: askRows.map(({ reviewId, ...ask }) => ({
      ...ask,
      reviewed: reviewId !== null,
      expired: ask.expiresAt.getTime() <= now,
    })),
  };
}
