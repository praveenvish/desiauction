...
async function main() {
  ...
  log("BETA SEASON CARD: " + (await p.getByTestId("season-pass").innerText()).replace(/\n+/g, " | "));

  // now put it on Free, as GA would
  await withDb(async (h) => h.sql`update competitions set tier='free' where slug=${slug}`);
  await p.reload({ waitUntil: "networkidle" });
  log("\nFREE SEASON CARD: " + (await p.getByTestId("season-pass").innerText()).replace(/\n+/g, " | "));
  ...
  await rd.getByLabel("What do you need it for? (optional)").fill("Twelve teams, auction on 14 Sep.");
  await rd.getByTestId("send-pass-request").click();
  await p.waitForTimeout(3000);
  log("\nTOAST: " + JSON.stringify((await p.locator("[role='status'],[role='alert']").allInnerTexts()).filter((t) => t.trim()).slice(0, 3)));
  await p.reload({ waitUntil: "networkidle" });
