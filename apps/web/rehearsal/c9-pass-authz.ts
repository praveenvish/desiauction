...
async function main() {
  ...
  const both = await signIn(b, "Demo Founder", "9999000001", { viewport: { width: 1280, height: 900 } });
  await probe(both.page, "admin+billing");
  await both.context.close();

  log("\n=== platform:admin ONLY (Demo Admin) ===");
  const adminOnly = await signIn(b, "Demo Admin", "9999000002", { viewport: { width: 1280, height: 900 } });
  await probe(adminOnly.page, "admin only");
  await adminOnly.context.close();

  log("\n=== no platform grant at all (Demo Organizer) ===");
  const none = await signIn(b, "Demo Organizer", "9999000003", { viewport: { width: 1280, height: 900 } });
